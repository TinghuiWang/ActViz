            ...
            using (var cmd = new NpgsqlCommand())
            {
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        string eventString = String.Format("{0:MM/dd/yyyy,HH:mm:ss,}{1},{2},,,{3}",
                                TimeZoneInfo.ConvertTime(reader.IsDBNull(0) ? DateTime.MinValue : reader.GetDateTime(0), testBed.TimeZone),
